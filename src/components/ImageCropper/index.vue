...
export default {
  props: {
    ...
    // The original name is key, similar to id, the trigger event will be brought on (if a page has multiple image upload controls, you can make a distinction
    ki: {
      'default': 0
    },
    ...
    // Does not display the rotation function
    noRotate: {
      type: Boolean,
      default: true
    },
  ...
  data() {
    ...
    return {
      ...
      loading: 0, // 0 not started 1 is being 2 successful 3 error
      progress: 0,
      ...
      sourceImgContainer: { // sic
        width: 240,
        height: 184 // If the generated graph scale is consistent with this, there will be a bug, first change to a special format, hahaha
      },
      ...
      scale: {
        ...
        rotateRight: false, // Button rotates to the right to open the event
  ...
  methods: {
    ...
    // Setup steps
    setStep(no) {
      // The delay is to show the animation effect, hahaha
      setTimeout(() => {
        this.step = no
      }, 200)
    },
  ...
  created() {
    // Binding button esc hides this plugin event
